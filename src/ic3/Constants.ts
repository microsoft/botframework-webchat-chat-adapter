const IC3_CHANNEL_ID = 'ic3';

export { IC3_CHANNEL_ID };

export const MissingAckFromPollingError = 10000;
export const ConnectionStatusObserverNotReadyError = 10001;
export const ReloadAllMessageInterval = 5 * 1000;
export const ConnectionStatusObserverWaitingTime = 4096;
export const Reinitialize = "reinitialize";
export const Translated = "translated";
export const TranslationMessageTag = "RTT-C1";